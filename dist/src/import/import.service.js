...
let ImportService = class ImportService {
    ...
    async importOuvriers(fileContent) {
        const parsed = Papa.parse(fileContent, { header: true, skipEmptyLines: true });
        if (parsed.errors.length > 0)
            throw new common_1.BadRequestException('Erreur de parsing CSV');
        const results = { success: 0, errors: [], skippedExisting: 0 };
    ...
    async importHabilitations(fileContent) {
        const parsed = Papa.parse(fileContent, { header: true, skipEmptyLines: true });
        const results = { success: 0, errors: [], skippedNotFound: 0 };
        for (const row of parsed.data) {
            try {
                const email = row.email || row.Email;
                ...
                if (!dateObtention || !dateExpiration) {
                    results.errors.push(`Dates invalides pour ${email}`);
                    continue;
                }
    ...
    async importEngins(fileContent) {
        const parsed = Papa.parse(fileContent, { header: true, skipEmptyLines: true });
        const results = { success: 0, errors: [], skippedExisting: 0 };
        for (const row of parsed.data) {
            try {
